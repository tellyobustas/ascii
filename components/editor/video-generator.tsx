      ...
      const response = await fetch("/api/telegram/send-result", {
        body: JSON.stringify({
          ...
          receipt: {
            generator: "VIDEO",
            parameters: [
              `mode / ${VIDEO_ASCII_PRESETS[presetId].mode}`,
              `fps / ${fps}`,
              `output / ${width}px`,
              `frames / ${result.video.renderedFrames}`,
              `duration / ${result.video.durationSeconds.toFixed(2)} sec`,
              result.video.wasTrimmed ? "source / auto trimmed to 15 sec" : "audio / stripped",
            ],
            preset: VIDEO_ASCII_PRESETS[presetId].label,
            renderId: "",
          },
          resultType: "videoMp4",
